...
ShowInfo(){
    ...
    Loop, % obj.MaxIndex()
    {
        ...
        ;~ key := RegExReplace(obj[A_Index]["key"], "<S-(.*)>", "$1")       ;注销，换下面内容
        ShiftKey:=obj[A_Index]["key"]
        OutK:=obj[A_Index]["key"]
...
/*  MyFun_RegExMatchAll   获取正则表达式所有匹配的数组
    Haystack				源字符串
    NeedleRegEx		正则表达式
    SubPat				
    ...
    用法:  
		...
		MyMatchArray:=MyFun_RegExMatchAll(MyOriStr,"o.*?\d")
		i:=1
...
Class __vim{
    __new()
    {
        ...
        this.LastFoundWin := ""
        This.HotKeyStr:=""        ;Kawvin修改，用于记录全部的热键字符串
        This.HelpStr:=""        ;Kawvin修改，用于记录热键功能说明
    }
    ...
    ;;【Kawvin】2018-04-01-19:57:41-----为输入法切换使用
    ; GetCurMode(winName) 
    ; 获取指定win中当前的模式
    GetCurMode(winName = "")
    {
    ...
    Map(keyName, Action, winName = "")
    {
        if not this.GetAction(Action)
        {
            ...
            {
                MsgBox % "映射按键【" keyname "】到功能【"  action  "】错误"
    ...
    Key()
    {
        ...
        if this.ExcludeWinList[winName]
        {
            ...
            This.HotKeyStr:=""      ;【Kawvin】2018-03-30-12:15:39
            return
        }
    ...
    */
    ...
    Convert2VIM(key)
    {
        ...
        if RegExMatch(key, "i)^lwin$")
            return "<Win>"
        if RegExMatch(key, "i)^LButton$")
            return "<LButton>"
        if RegExMatch(key, "i)^~LButton\s&\s(.*)", m)  ; 添加先按鼠标在按按键 by_BoBO 20190820
           return "<LB-" this.Upper(m1) ">"
        if RegExMatch(key, "i)^CapsLock\s&\s(.*)", m)  ; 添加Cap组合 by_BoBO 20190820
            return "<Caps-" this.Upper(m1) ">"
        if RegExMatch(key, "i)^~Tab\s&\s(.*)", m)  ; 添加Tab组合 by_BoBO 20190820
            return "<Tab-" this.Upper(m1) ">"
        ; if RegExMatch(key, "i)^~lctrl$")
        ;     return "<~LC>"
        ; if RegExMatch(key, "i)^~lshift$")
        ;     return "<~LS>"
        return key
    }
    ...
    Convert2AHK(key, ToSend=false)
    {
        ...
        if RegExMatch(key, "^<.*>$")
        {
            ...
            if RegExMatch(key, "i)^LButton$")
                return ToSend ? "{LButton}" : "LButton" ; 避免冲突 取消{} BoBO
            if RegExMatch(key, "i)^alt$")
                return ToSend ? "!" : "alt" ; 避免冲突 取消{} BoBO
            if RegExMatch(key, "i)^ctrl$")
                return ToSend ? "^" : "ctrl" ; 避免冲突 取消{} BoBO
            if RegExMatch(key, "i)^shift$")
                return ToSend ? "+" : "shift" ; 避免冲突 取消{} BoBO
            ...
            if RegExMatch(key, "i)^LB\-(.*)", m)
               return ToSend ? "{~LButton}" this.CheckToSend(m1) : "~LButton & " m1 ;  BoBO 20190820
            if RegExMatch(key, "i)^Caps\-(.*)", m)
                return ToSend ? "{Caps}" this.CheckToSend(m1) : "CapsLock & " m1 ;  BoBO 20190820
            if RegExMatch(key, "i)^Tab\-(.*)", m)
                return ToSend ? "{~Tab}" this.CheckToSend(m1) : "~Tab & " m1 ;  BoBO 20190820
            
            ; if RegExMatch(key, "i)^~LC")
            ;     return ToSend ? "{~}^" : "~lctrl"
            ; if RegExMatch(key, "i)^~LS")
            ;     return ToSend ? "{~}+" : "~lshift"
