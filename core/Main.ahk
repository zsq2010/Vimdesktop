...
CheckPlugin(){
    ...
    {
        IfExist, %A_ScriptDir%\plugins\%plugin%\%plugin%.ahk
        {
            if flag
                vim.LoadPlugin(plugin)
        } else {
            ini.DeleteKey("plugins",plugin)
            ini.DeleteKey("plugins_DefaultMode",plugin)
            ini.save()
        }
    }
    
    for plugin, mode in ini.plugins_DefaultMode ;切换到默认模式
