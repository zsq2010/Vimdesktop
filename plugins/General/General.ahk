General:
    ...
    ;BoBO定义激活内容
    vim.SetAction("<RunEverything>", "运行 Everything")
    vim.SetAction("<RunAE>", "运行 AdobeAfterEffect")
    vim.SetAction("<RunPS>", "运行 Photoshop")
    vim.SetAction("<RunMax>", "运行 3DsMax")
    vim.SetAction("<RunFirefox>", "运行 Firefox")
    vim.SetAction("<RunGoogleChrome>", "运行 Google Chrome")
    vim.SetAction("<RunVivaldi>", "运行 Vivaldi")
    vim.SetAction("<RunUnity3D>", "运行 U3D")
...
<RunZ>:
    ...
    if (FileExist(A_ScriptDir "\vimd.ahk"))
    {
        Run, %A_ScriptDir%\vimd.ahk "%RunZPath%"
    }
    else
    {
        Run, "%RunZPath%"
...
; 带图标
; #g::
; run %COMMANDER_PATH%\Tools\popsel\PopSel.exe /is /pc /n
; 不带图标
#g::
