TotalCommander:
    ...
    vim.Comment("<TC_LeftSelect>", "向左选择")
    vim.Comment("<TC_RightSelect>", "向右选择")
    vim.Comment("<TC_HomeSelect>", "选择到头")
    vim.Comment("<TC_EndSelect>", "选择到尾")
    ...
    vim.Comment("<TC_SlectOrUnslect>","同空格,选择/反选 光标所在文件")
    vim.Comment("<TC_PackFiles>","调用win菜单压缩")
    vim.Comment("<TC_UnpackFiles>","调用win菜单解压")
    vim.Comment("<TC_StyleC40000>","显示/隐藏TC标题栏")
    vim.Comment("<TC_BZPackFiles>","调用BZ菜单压缩")
    vim.Comment("<TC_BZUnpackFiles>","调用BZ菜单解压")
    vim.Comment("<TC_SwitchWindows>","切换窗口")
    vim.Comment("<TC_Help>","帮助，查看热键")    
    vim.Comment("<TC_SearchStartOrOver>", "开始/停止搜索")
    vim.Comment("<TC_ClickFileSearch>","点击文件搜索框")
    vim.Comment("<TC_FileSearchEV>","文件EV搜索")     
    vim.Comment("<TC_FileSearchTC>","文件TC搜索")
    vim.Comment("<TC_OutputList>","输出到列表")
    vim.Comment("<TC_NewSearch>","新的搜索")
    vim.Comment("<TC_TextSearchOpen>","文本搜索打开/关闭")
    vim.Comment("<TC_FileUnlock>","文件解锁")      
    vim.Comment("<TC_FileUnlockDelete>","解锁删除")      
    vim.Comment("<TC_TextSearch>","文本搜索")      
...
;#include *i %A_ScriptDir%\plugins\TotalCommander\TCCOMMAND.ahk

    
    vim.SetWin("TCFindFile", "TFindFile")
    vim.mode("normal", "TCFindFile")
    vim.map("As", "<TC_SearchStartOrOver>", "TCFindFile")
    vim.map("Ac", "<TC_ClickFileSearch>", "TCFindFile")
    vim.map("Ae", "<TC_FileSearchEV>", "TCFindFile")
    vim.map("At", "<TC_FileSearchTC>", "TCFindFile")   
    vim.map("Af", "<TC_OutputList>", "TCFindFile")
...
;命令模式，搜索模式    
 ...
 #include *i %A_ScriptDir%\plugins\TotalCommander\TC_SearchMode.ahk    
    vim.map("<esc>", "<TC_NormalMode>", "TTOTAL_CMD")
;可视模式
...
;普通模式开始
    ...
    #include *i %A_ScriptDir%\plugins\TotalCommander\TC_NormalMode.ahk
...
<Tc_DoubleSpace>:
{
    ...
    settimer, TC_tappedkey_Space, %t%

    if (t == "off")
    goto TC_double_Space
    return
...
;开启/关闭文本搜索
<TC_TextSearchOpen>:
CoordWinClick(69,228)
    return
;新的搜索
<TC_NewSearch>:
CoordWinClick(488,938)
    return
;输出到列表
<TC_OutputList>:
CoordWinClick(824,934)
    return
;文件搜索TC模式
<TC_FileSearchTC>:
CoordWinClick(183,97)
    Sleep,50
    SendInput,^a
    Sleep,100
    SendInput,{Delete}
...
CoordWinClick(183,97)
    return
;开始搜索
<TC_SearchStartOrOver>:
CoordWinClick(911,80)
    return
;TC撤销，win7以上需要做一些修改
<TC_Undo>:
;点击窗口内指定坐标
ControlClick, x100 y100, ahk_class Progman, FolderView
;向控件发送热键
...
SendPos(2018)
Sleep,100
fileunlock:= Clipboard
Run, %TCDirPath%\Tools\IObitUnlocker\IObitUnlocker.exe /None "%fileunlock%"
return
...
Sleep,100
fileunlock:= Clipboard
Run, %TCDirPath%\Tools\IObitUnlocker\IObitUnlocker.exe /Delete "%fileunlock%"
return
...
<TC_BZUnpackFiles>:
 ...
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_Bandizip-AutoExt_Y`)"
 ;~ Sleep,200
;~ SendPos(2029)
;~ Sleep,200
...
Sleep,100
Srcpath:= Clipboard
Run, C:\Windows\explorer.exe "%Srcpath%"
return
...
<TC_CopyDirectoryHotlist>:
    ControlGetFocus, CurrentFocus, AHK_CLASS TTOTAL_CMD
;2018-10-15 23:50:41出现问题把这里的2、1改成了4、3
    if CurrentFocus not in TMyListBox2, TMyListBox1
;        MsgBox, nono tmy12
        return
    if CurrentFocus in TMyListBox2
...
;<cm_SpreadSelection2>: >>创建当前光标下文件的快捷方式
<TC_SlectOrUnslect>:
    Send,{Space}
Return
...
;<TC_MoveDirectoryCurrent>: >>将当前选择的文件夹下的所有文件移动到当前目录，并删除原目录
;~ <TC_MoveDirectoryCurrent>:
;~ SendPos(2018)
;~ run , ".\apps\FileMove\move_here.cmd" 
;~ return
...
TCCOMMAND:
    ...
    vim.Comment("<cm_GotoPreviousDir>", "后退,同鼠标的后退")
    vim.Comment("<cm_GotoNextDir>", "前进,同鼠标的后退")
...
<cm_SearchFor>:
    SendPos(501)
    WinWait,ahk_class TFindFile,,10
    ControlClick,x259 y92,ahk_class TFindFile,,,,Pos
    Sleep,50
    SendInput,^a
    Sleep,100
    SendInput,% uStr("ev:")
...
{   
    t := A_PriorHotkey == A_ThisHotkey && A_TimeSincePriorHotkey < 200 ? "off" : -200
    settimer, tc_tappedkey_Clear, %t%

    if (t == "off")
    goto tc_double_Clear
    return
...
{   
    t := A_PriorHotkey == A_ThisHotkey && A_TimeSincePriorHotkey < 200 ? "off" : -200
    settimer, tc_tappedkey_Compare, %t%

    if (t == "off")
    goto tc_double_Compare
    return

    tc_tappedkey_Compare:
    Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`cm_CompareFilesByContent`)"
    return

    tc_double_Compare:
    Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`cm_IntCompareFilesByContent`)"
    return
return
}
;TCIMG一些命令
;搜索菜单命令
<TC_TCIMG_SearchRunMenu>:
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_TCIMG_SearchRunMenu`)"
return
;一键并入并行目录
<TC_TCIMG_GoParDir>:
...
;FastCopy快速复制CVT/SSP..
<TC_FastCopy_CopyCVT>:
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_fastcopy_copyCVT`)"
return
;FastCopy快速复制Info&Extra..
<TC_FastCopy_CopyInfo>:
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_fastcopy_copyInfo&Extra`)"
return
;FastCopy快速复制
<TC_FastCopy_Copy>:
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_fastcopy_copy`)"
return
;FastCopy快速移动
<TC_FastCopy_Move>:
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_fastcopy_move`)"
return
;FastCopy快速删除
<TC_FastCopy_Delete>:
 Run "%TCDirPath%\Tools\TCFS2\TCFS2.exe" /ef "tem(`em_fastcopy_delete`)"
return
;FastCopy快速删除PNG
<TC_FastCopy_DeletePNG>:
